...
export default function CustomButton({ label = "Continue", to = null, onClick, disabled = false }) {
  ...
  return (
    <div
      onClick={handleClick}
      className={`cursor-pointer w-full max-w-[514.99px] h-[50px] sm:h-[52px] md:h-[54px] relative rounded-lg mt-4 sm:mt-5 md:mt-6 mx-auto transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
    >
