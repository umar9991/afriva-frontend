...
export default function PasswordField({ placeholder, value, onChange, className = '' }) {
  const [showPassword, setShowPassword] = useState(false);

  const togglePassword = () => setShowPassword(prev => !prev);

  return (
    <div className={`w-full max-w-md sm:max-w-lg md:max-w-xl h-[48px] sm:h-[50px] md:h-[52px] px-3 sm:px-4 md:px-5 py-2.5 sm:py-3 md:py-3.5 rounded-md outline outline-1 outline-offset-[-1px] outline-[#e0e2e9] hover:outline-[#c0c4d0] focus-within:outline-[#8BC53F] focus-within:outline-2 transition-all duration-200 inline-flex justify-between items-center gap-2.5 ${className}`}>
    <input
